...
# Images need to be shaped into 128x128x3
...
# monochromatic images were resized to RGB and have  in_channels = 3, it was done during the preprocessing
...
##question encoder LSTM net
    ...
    def forward(self, question):
        ...
        question_v = self.wordtovec(question)                             # [batch_size, q_voc_size=26, word_emb_size=300]
        question_v = self.tanh(question_v)
        question_v = question_v.transpose(0, 1)                             # [q_voc_size=26, batch_size, word_emb_size=300]
        ...
        question_feature = self.fc(question_feature)                            # [batch_size, emb_size]
...
class VQA_NET(nn.Module):
    ...
    def forward(self, img, question):
        #print("img size")
        #print(img.size())
        image_f = self.image_encoder(img)                     # [batch_size, emb_size]
        question_f = self.question_encoder(question)                     # [batch_size, emb_size]
        comb_feature = torch.mul(image_f, question_f)  # [batch_size, emb_size]
