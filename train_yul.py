...
def main():
    ...
    optimizer = optim.Adam(
        ...
        #list(model.image_encoder.vgg_model.parameters()) + list(model.question_encoder.parameters()) + list(
         #   model.fc1.parameters()) + list(model.fc2.parameters()), lr=learning_rate)
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(VQA_NET, os.path.join(data_path, f'model-epoch-{epoch + 1}.pkl'))

    print("all epochs time minutes")
    print((time.time() - start_time) / 60)

    ### Save the model
    torch.save(VQA_NET, './model_hw2_test1.pkl')
